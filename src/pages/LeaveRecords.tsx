...
export default function LeaveRecords() {
  ...
  return (
    <div>
      ...
      {error && <AlertBar t={t} type="crit">Failed to load leave records.</AlertBar>}
          ...
          <MobileList
            ...
            renderItem={(r) => (
              <div>
                ...
                <div style={{ fontSize: 11, color: t.textMuted, marginTop: 4 }}>{r.span_start}–{r.span_end}</div>
              </div>
            )}
          />
        ) : (
          <RTable t={t} bp={bp} cols={cols} rows={records} rowKey={(r) => r.id} />
        )
      )}

      {waitlist.length > 0 && (
        <Card t={t} style={{ marginTop: 20 }}>
          <h3 style={{ fontSize: 13, color: t.text, marginTop: 0, marginBottom: 10 }}>Waitlist (FIFO order)</h3>
          {slotLedger && (
          ...
          {waitlist.map((r) => (
            <div
              key={r.id}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: `1px solid ${t.border}` }}
            >
              ...
              <button
                type="button"
                onClick={() => updateStatus.mutate({ id: r.id, status: 'Granted' })}
                style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: t.ok, color: '#fff', fontSize: 12, cursor: 'pointer' }}
              >
                Approve
              </button>
            </div>
